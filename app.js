...
function renderCasCard(i, query) {
  ...
  function buildGroup(links) {
    const all = links || [];
    if (!q) return all;
    const matched = all.filter(l => l.label.toLowerCase().includes(q));
    return matched.length ? matched : all;
  }

  const adminRows = buildGroup(i.adminLinks).map(l => linkRow(l.label, l.url, ICONS[l.icon] || ICONS.form)).join('');
  const systemRows = buildGroup(i.systemLinks).map(l => linkRow(l.label, l.url, ICONS[l.icon] || ICONS.form)).join('');
  const driveRows = buildGroup(i.driveLinks).map(l => linkRow(l.label, l.url, ICONS.folder)).join('');
  const mapRows = buildGroup(i.mapLinks).map(l => linkRow(l.label, l.url, ICONS[l.icon] || ICONS.map)).join('');
  ...
  return `
    <div class="tech-card cas-card">
      ...
      <div class="card-body">
        ...
        <div class="cas-section-title">${ICONS.folder} Atalhos e Sistemas</div>
        <div class="cas-groups">
          ...
          <div class="cas-group">
            <div class="cas-group-header">Sistemas 2026</div>
            <div class="cas-link-list">
              ${systemRows || emptyRow}
            </div>
          </div>

          <div class="cas-group" style="grid-column: 1 / -1;">
        ...
        <div class="cas-groups">
          <div class="cas-group" style="grid-column: 1 / -1;">
            <div class="cas-group-header">Mapa do Território</div>
            <div class="cas-link-list">
              ${mapRows || emptyRow}
            </div>
          </div>
...
function renderCrasCard(i, query) {
  ...
  const rmaRows = buildGroup(i.rmaLinks).map(l => linkRow(l.label, l.url, ICONS[l.icon] || ICONS.form)).join('');
        ...
        <div class="team-tables">
          <div class="team-subcard">
            ...
            <div class="team-subcard-note">
              <strong style="display:block; margin-bottom:0.4rem; color:var(--text-main); font-size:0.7rem; text-transform:uppercase;">Abrangência Geral</strong>
              <div class="team-tag-list">
                ${(i.fixedCoverage || []).map(b => `<span class="team-tag">${b}</span>`).join('')}
              </div>
            </div>
          </div>
    ...
    <div class="tech-card cras-card">
      ...
      <div class="card-body">
        ...
        <div class="cras-groups">
          ...
          <div class="cras-group">
            <div class="cras-group-header">Gestão de Atendimentos (RMA)</div>
            <div class="cras-link-list">
              ${rmaRows || emptyRow}
            </div>
          </div>
      ...
      <div class="card-body">
        ...
        <div class="pdftools-tool" style="margin-bottom:1.1rem;">
          <div class="pdftools-tool-header">
            ...
            <div>
              <div class="pdftools-tool-title">Unificar PDF</div>
          ...
          <label class="pdftools-dropzone" for="pdftoolsMergeInput" ondragover="event.preventDefault(); this.classList.add('dragover')" ondragleave="this.classList.remove('dragover')" ondrop="pdftoolsHandleDrop(event, 'merge')">
            ...
            <div class="pdftools-dropzone-text">Toque para escolher os PDFs</div>
...
async function printGuide(id) {
  ...
  const printArea = document.getElementById('print-area');
  printArea.innerHTML = firstPage + secondPage + pdfPage;
...
function importData(fileInput) {
  ...
  reader.onload = function (e) {
    try {
      ...
      const dados = payload && typeof payload.dados === 'object' && payload.dados ? payload.dados : null;
      ...
      alert('Não foi possível importar este arquivo. Verifique se é um backup gerado pelo próprio Argo SUAS (botão "Backup de anotações").');
    } finally {
      fileInput.value = '';
    }
  };
...
const PDFTOOLS_MERGE_MAX_FILES = 10;
const PDFTOOLS_MERGE_MAX_BYTES = 50 * 1024 * 1024;
const pdftoolsMergeState = { files: [] };
...
async function pdftoolsAddMergeFiles(fileList) {
  ...
  try {
    ...
    pdftoolsSetStatus('pdftoolsMergeStatus', 'Não foi possível carregar a biblioteca de PDF. Verifique sua conexão.', 'error');
    return;
  }
  for (const file of files) {
    if (pdftoolsMergeState.files.length >= PDFTOOLS_MERGE_MAX_FILES) {
      pdftoolsSetStatus('pdftoolsMergeStatus', `Limite de ${PDFTOOLS_MERGE_MAX_FILES} arquivos atingido.`, 'warn');
      break;
    }
    ...
    try {
      ...
      pdftoolsSetStatus('pdftoolsMergeStatus', `Não foi possível ler "${file.name}" (arquivo protegido ou corrompido).`, 'error');
    }
  }
  ...
  list.innerHTML = pdftoolsMergeState.files.map((f, idx) => `
    <li class="pdftools-fileitem">
      ...
      <span class="pdftools-fileitem-btns">
        <button type="button" class="pdftools-icon-btn" title="Mover para cima" ${idx === 0 ? 'disabled' : ''} onclick="pdftoolsMoveMergeFile('${f.id}', -1)">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="18 15 12 9 6 15"/></svg>
        </button>
        <button type="button" class="pdftools-icon-btn" title="Mover para baixo" ${idx === pdftoolsMergeState.files.length - 1 ? 'disabled' : ''} onclick="pdftoolsMoveMergeFile('${f.id}', 1)">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"/></svg>
        </button>
        <button type="button" class="pdftools-icon-btn pdftools-remove" title="Remover" onclick="pdftoolsRemoveMergeFile('${f.id}')">
...
async function pdftoolsRunJpgToPdf() {
  ...
  try {
    ...
    pdftoolsSetStatus('pdftoolsJpgPdfStatus', 'PDF gerado com sucesso!', 'success');
  } catch (e) {
    pdftoolsSetStatus('pdftoolsJpgPdfStatus', 'Erro ao converter as imagens. Verifique se são JPG/PNG válidos.', 'error');
  } finally {
    if (btn) btn.disabled = pdftoolsJpgPdfState.files.length === 0;
  }
}
